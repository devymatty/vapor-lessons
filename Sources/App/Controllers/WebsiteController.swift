...
struct WebsiteController: RouteCollection {
    ...
    func categoryHandler(_ req: Request) throws -> Future<View> {
        return try req.parameters.next(Category.self).flatMap(to: View.self) { category in
            let acronims = try category.acronyms.query(on: req).all()
            let context = CategoryContext(title: category.name, category: category, acronyms: acronims)
                                ...
                                return acronym.save(on: req)
                                    .flatMap(to: [Category].self) { _ in
                                        ...
                                        for newCategory in categoriesToAdd {
                                            categoryResult.append(try Category.addCategory(newCategory, to: acronym, on: req))
                                        }
                                        ...
                                        for categoryNameToRemove in categoriesToRemove {
                                            ...
                                            if let category = categoryToRemove {
                                                categoryResult.append(acronym.categories.detach(category, on: req))
                                            }
                                        }
